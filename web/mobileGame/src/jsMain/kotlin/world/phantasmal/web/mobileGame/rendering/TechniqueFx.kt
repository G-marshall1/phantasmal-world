...
class TechniqueFx(
    ...
    private fun flameBody(radiusUnits: Double): Object3D {
        ...
        // The body itself churns: the noise drives the vertices, not just the shading, so the
        // surface boils and throws tongues instead of being a sphere with fire painted on it.
        val flame = Mesh(
            ...
            ShaderMaterial(
                obj {
                    uniforms = obj {
                        uTime = obj { value = 0.0 }
                        uDisplace = obj { value = radiusUnits * bu * FOIE_CORE_CHURN }
                    }
        ...
        // A looser, wispier shell around it, churning harder and on its own clock -- what
        // gives the fire an edge that moves rather than a hard silhouette.
        val shell = Mesh(
            IcosahedronGeometry(radiusUnits * FOIE_SHELL_SCALE * bu, FOIE_FLAME_DETAIL - 1),
            ShaderMaterial(
                obj {
                    uniforms = obj {
                        uTime = obj { value = 0.0 }
                        uDisplace = obj { value = radiusUnits * bu * FOIE_SHELL_CHURN }
                    }
                    vertexShader = FOIE_VERTEX_SHADER
                    fragmentShader = FOIE_SHELL_FRAGMENT_SHADER
                    transparent = true
                    depthWrite = false
                    blending = AdditiveBlending
                    side = DoubleSide
    ...
    companion object {
        ...
        /** How far the noise pushes the surface, as a fraction of the flame's own radius. */
        const val FOIE_CORE_CHURN = 0.22
        const val FOIE_SHELL_CHURN = 0.34
        const val FOIE_SHELL_SCALE = 1.5
        ...
        /**
         * Simplex noise, the compact Ashima/Gustavson form. Worth its size over the value
         * noise this replaced: it has no grid artefacts, which is what lets it drive the
         * geometry as well as the colour without the sphere showing its lattice.
         */
        const val SIMPLEX_NOISE = """
        ...
        /** Pushes each vertex along its own normal by the noise field, so the body boils. */
        const val FOIE_VERTEX_SHADER = """
            ...
            void main() {
                ...
                float churn = snoise(normal * 3.0 + vec3(0.0, uTime * 0.9, uTime * 0.5));
                vec3 shifted = position + normal * churn * uDisplace;
                gl_Position = projectionMatrix * modelViewMatrix * vec4(shifted, 1.0);
            }
        """

        /** The shell: sparse filaments and a soft rim, so it veils the core without hiding it. */
        const val FOIE_SHELL_FRAGMENT_SHADER = """
            ...
            """ + SIMPLEX_NOISE + """
            void main() {
                float n = snoise(vPos * 4.0 + vec3(uTime * 0.6, uTime * 0.4, 0.0));
                float filament = smoothstep(0.15, 0.85, n);
                float fres = pow(1.0 - abs(dot(normalize(vNormal), vec3(0.0, 0.0, 1.0))), 1.4);
                vec3 color = vec3(1.0, 0.45, 0.08) * (0.25 + 1.5 * filament + 0.8 * fres);
                float alpha = clamp(0.05 + 0.45 * filament + 0.35 * fres, 0.0, 0.75);

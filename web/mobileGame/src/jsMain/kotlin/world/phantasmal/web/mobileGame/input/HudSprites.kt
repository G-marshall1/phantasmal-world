 ...
 *
 * The cells are the game's real item-reader plates -- the green hex icons the item reader
 * shows for each tool -- for everything that has one; the materials, grinders, disks and
 * trade trophies have no reader plate anywhere, so those cells are drawn in the HUD sheet's
 * pixel style instead (per-stat gems, tiered gears, the violet disk, the golden wing). One
 * cell per ToolType in declaration order, then the technique disk and the trade-trophy wing
 * (see ItemIcons.kt for the index mapping). Built by web/assets-generation/gen_tool_icons.py
 * from the plates mirrored under wiki-item-icons/.
 *
 * The sheet's cells are 64px but the style below speaks in 16px units like [itemIconStyle]:
 * background-size scales the whole sheet, so the source resolution never leaks out. No
 * pixelated rendering -- the plates are smooth art, and the drawn cells were upscaled

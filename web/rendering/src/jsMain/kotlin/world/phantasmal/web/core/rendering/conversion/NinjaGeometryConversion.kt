...
import world.phantasmal.web.externals.three.Uint32BufferAttribute
...
fun collisionGeometryToGroup(
    ...
    for (collisionMesh in collisionGeometry.meshes) {
        ...
        val materialGroups = mutableMapOf<Int, JsArray<Int>>()
        var index = 0
        ...
        if (index > 0) {
            ...
            // Each accepted triangle contributes 3 fresh, non-shared vertices (no de-dup against
            // other triangles), so a large map's walkable subset alone can push the total past
            // Uint16's 65535 ceiling -- same overflow hazard as MeshBuilder.build, see its
            // comment for why the concrete typed array type (not just byte width) has to match.
            val useWideIndices = index > 65535
            val indices16 = if (useWideIndices) null else Uint16Array(index)
            val indices32 = if (useWideIndices) Uint32Array(index) else null
            var offset = 0

            for ((materialIndex, vertexIndices) in materialGroups) {
                if (useWideIndices) {
                    indices32!!.set(vertexIndices.asArray(), offset)
                } else {
                    indices16!!.set(vertexIndices.asArray().map { it.toShort() }.toTypedArray(), offset)
                }
